        ...
        function getActiveFinishingList() {
            ...
            if (isChecked('f-zgibanje-active')) list.push('Zgibanje');
            ...
            if (isChecked('f-lam-active')) list.push('Plastifikacija');
        ...
        function calculateForSingleQty(qty, sheetsNeededIn, drawW, drawH, orderedQty) {
            ...
            let paperPrice = parseFloat(document.getElementById('calc-paper-price').value) || 0;
            let paperUnit = document.getElementById('calc-paper-unit').value;
            let paperWeight = parseFloat(document.getElementById('calc-paper-weight').value) || 0;
            ...
            let physicalWasteSheets = _isDoubleSidedForMat ? Math.ceil(paperWasteSheets / 2) : paperWasteSheets;
            ...
            // Izračunamo koliko OSNOVNIH pol potrebujemo
            ...
            if (speedMaxInpVal !== "") {
                ...
                mSpeed = getDynamicSpeed(rawMachineSpeed, paperWeight, qty);
            ...
            // Posodabljamo prikazovalno polje z dejansko izračunano hitrostjo
            ...
            let mPrepMinutes = 10; // Privzeto osnovno
            if (mPrepInp !== "") {
                mPrepMinutes = parseFloat(mPrepInp) || 0;
            } else {
                // Boss formula za pripravo:
                // Osnovna priprava (10 min) + Menjava plošč (6 min na agregat/ploščo) + Obračalnik (15 min)
                mPrepMinutes = 10 + (6 * platesNum) + (isObrat ? 15 : 0);
                const prepTimeInp = document.getElementById('calc-machine-prep-time');
                if (prepTimeInp) {
                    prepTimeInp.placeholder = "Avto (" + mPrepMinutes + " min)";
                }
            }
            ...
            if (document.getElementById('f-uv-active').checked) {
                ...
                let uvSheetsCalculated = sheetsNeeded + (extraWaste / 2);
            ...
            if (document.getElementById('f-uv-active').checked) {
                ...
                details.finish.items.push({
                    name: 'UV lak (tuja)',
                    cost: uCost,
                    breakdown: `Priprava: ${formatPrice(uPrep)} | Delo: ${sheetsNeeded} (neto) + ${extraWaste}/2 (tiskar) = ${uvSheetsCalculated.toFixed(0)} pol po ${pricePerSheet.toFixed(3)} € (${formatPrice(uWork)})`
                });
            }
            if (document.getElementById('f-personalization-active') && document.getElementById('f-personalization-active').checked) {
                let isBothSides = document.getElementById('f-personalization-both-sides') && document.getElementById('f-personalization-both-sides').checked;
                let pMultiplier = (mType === 'S8') ? 2 : 1;
                if (isBothSides) pMultiplier *= 2; // Obojestransko = 2x več pol
                let pSheets = sheetsNeeded * pMultiplier;
                let pPrice = (parseFloat(document.getElementById('f-personalization-price').value) || 0);
                let pCost = pSheets * pPrice;
                let sidesText = isBothSides ? ' (obe strani)' : '';
                let breakdownText = `Delo: ${pSheets} pol${sidesText} po ${formatPrice(pPrice)}/polo = ${formatPrice(pCost)}`;
                let extraCost = 0;
                // Preveri ali je B2 format (698x498) in dodaj razrez na 318x448
            ...
            if (!layout.isMixed) {
                for (let tempC = 1; tempC <= layout.cols; tempC++) {
                    ...
                    if (tempR <= layout.rows) {
                        ...
                        let bestDiff = Math.abs(bestC - bestR);
                        if (isObratChecked) {
                            if (bestC % 2 !== 0) bestDiff += 1000;
                            if (tempC % 2 !== 0) diff += 1000;
                        }
                        if (diff < bestDiff || (diff === bestDiff && tempC > bestC)) {
                            bestC = tempC;
                            bestR = tempR;
                        }
                    }
            ...
            try {
                ...
                const data = {
                    ...
                    inputs: {
                        ...
                        finish: {
                            cilinder: { active: gc('f-cilinder-active'), prep: gv('f-cilinder-prep'), hours: gv('f-cilinder-hours'), rate: gv('f-cilinder-rate') },
                            ...
                            personalization: { active: gc('f-personalization-active'), price: gv('f-personalization-price') },
        ...
        async function saveProjectToFile() {
            try {
                ...
                const customer = document.getElementById('calc-customer') ? document.getElementById('calc-customer').value : '';
                let tParts = [];
                if (quoteNum && quoteNum !== '/') tParts.push(quoteNum);
                if (customer) tParts.push(customer);
                if (name) tParts.push(name);
                if (fFormat) tParts.push(fFormat);
                if (cColors) tParts.push(cColors);
                if (nakladaStr) tParts.push(nakladaStr + "kos");
                const suggestedName = 'pon._' + tParts.join('_').replace(/[/\\?%*:|"<> \t]/g, '_').replace(/_+/g, '_') + '.json';
        ...
        function renderSavedProjects() {
            ...
            if (filtered.length > 0) {
                ...
                filtered.forEach(proj => {
                    ...
                    const sourceFormatDisplay = (proj.inputs && proj.inputs.sourceW && proj.inputs.sourceH) ? ` | Osnovni format: ${proj.inputs.sourceW}x${proj.inputs.sourceH}` : "";
                    html += `
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 12px; border-bottom: 1px solid #334155; transition: background 0.2s;" class="project-item-row" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
        ...
        function loadProjectData(proj) {
            function setV(id, val, isChk = false) {
                ...
                if (!el) return;
                if (isChk) el.checked = !!val; else el.value = val;
            ...
            if (inp.sourcePreset !== undefined) document.getElementById('calc-source-presets').value = inp.sourcePreset;
            ...
            if (inp.sourceYield !== undefined) document.getElementById('calc-source-yield').value = inp.sourceYield;
            ...
            if (document.getElementById('calc-is-sv')) document.getElementById('calc-is-sv').checked = inp.isSV || false;
            ...
            if (inp.finish) {
                ...
                if (f.cilinder) {
                    ...
                    document.getElementById('f-cilinder-hours').value = f.cilinder.hours !== undefined ? f.cilinder.hours : '';
                ...
                if (f.razrezFormat) {
                    ...
                    document.getElementById('f-razrez-format-speed').value = f.razrezFormat.speed || "";
                    document.getElementById('f-razrez-format-rate').value = f.razrezFormat.rate || "";
                ...
                if (f.personalization) {
                    if (document.getElementById('f-personalization-active')) document.getElementById('f-personalization-active').checked = f.personalization.active;
                    if (document.getElementById('f-personalization-price')) document.getElementById('f-personalization-price').value = f.personalization.price || 0.03;
                            ...
                            <tr>
                                ...
                                <td style="width: 85%; vertical-align: top; padding-left: 10px;">
                                    <table style="width: 100%; border-collapse: collapse;">
                                        <tr><td style="width: 100px; font-weight: bold; font-size: 12px; vertical-align: top;">${isAT ? 'Produkt:' : 'Izdelek:'}</td><td style="font-size: 12px; vertical-align: top;">${item.name || '/'}</td></tr>
                                        <tr><td style="width: 100px; font-weight: bold; font-size: 12px; vertical-align: top;">Format:</td><td style="font-size: 12px; vertical-align: top;">${(item.spec && item.spec.format) ? item.spec.format : '/'}</td></tr>
                                        ${(item.spec && item.spec.paper) ? `<tr><td style="width: 100px; font-weight: bold; font-size: 12px; vertical-align: top;">${isAT ? 'Papier:' : 'Papir:'}</td><td style="font-size: 12px; vertical-align: top;">${item.spec.paper}</td></tr>` : ''}
                                        <tr><td style="width: 100px; font-weight: bold; font-size: 12px; vertical-align: top;">${isAT ? 'Druck:' : 'Tisk:'}</td><td style="font-size: 12px; vertical-align: top;">${(item.spec && item.spec.colors) ? item.spec.colors : '/'}</td></tr>
                                        ${(item.spec && item.spec.finishing) ? `<tr><td style="width: 100px; font-weight: bold; font-size: 12px; vertical-align: top;">${isAT ? 'Verarbeitung:' : 'Dodel.:'}</td><td style="font-size: 12px; vertical-align: top;">${item.spec.finishing}</td></tr>` : ''}
                                        ${deliveryAddress ? `<tr><td style="width: 100px; font-weight: bold; font-size: 12px; vertical-align: top;">${isAT ? 'Lieferung:' : 'Dostava:'}</td><td style="font-size: 12px; vertical-align: top;">${deliveryAddress.replace(/\n/g, '<br>')}</td></tr>` : ''}
                                    </table>
                                </td>
                            </tr>
                        </table>
            ...
            const defaultContent = `
                    ...
                    <table style="width: 100%; margin-bottom: 5px;" cellpadding="0" cellspacing="0">
                        <tr>
                            ...
                            <td style="width: 50%; vertical-align: top;" align="right"><table class="info-table" cellpadding="0" cellspacing="0"><tr><td>Datum</td><td>${date}</td></tr><tr><td>${isAT ? 'Kunde' : 'Šifra stranke'}</td><td>${customerCode}</td></tr><tr><td>${isAT ? 'Angebot' : 'Ponudba'}</td><td>${quoteNum}</td></tr></table></td>
                        </tr>
                    </table>
        ...
        function getWorkOrderHTML() {
            ...
            // Preberi dodatek iz polja "Dodatne pole tiskarju" (ID: calc-given-sheets)
            ...
            let printMathString = `${calcRes.sheetsNeeded.toLocaleString('de-DE')} + ${finalDodatek.toLocaleString('de-DE')} = ${finalTotalSheets.toLocaleString('de-DE')} tisk. pol`;
            if (!isNaN(givenDodatekInput) && givenDodatekInput >= 0) {
                ...
                printMathString = `${calcRes.sheetsNeeded.toLocaleString('de-DE')} pol + ${finalDodatek.toLocaleString('de-DE')} pol = ${finalTotalSheets.toLocaleString('de-DE')} pol`;
            }
            ...
            const defaultContent = `
                ...
                <table style="margin-top: 5px; width: 100%;">
                    ...
                    <tr>
                        <td>Tisk:</td>
                        <td class="bold" style="width: 55%; white-space: nowrap;">${printMathString} &nbsp;&nbsp; (${mPasses}x skozi stroj)</td>
            ...
            <body>
                <div class="no-print" contenteditable="false" style="background: #f1f5f9; padding: 10px; border-bottom: 1px solid #cbd5e1; display: flex; gap: 10px; align-items: center; justify-content: start; font-family: sans-serif; box-sizing: border-box; width: 100%; flex-wrap: wrap;">
                    ...
                    <button onclick="window.close()" style="background: #64748b; color: white; border: none; padding: 8px 16px; border-radius: 4px; font-weight: bold; cursor: pointer; font-size: 13px;">ZAPRI</button>
                    <button onclick="if(confirm('Ali želite ponastaviti delovni nalog na privzete vrednosti? (Spremembe besedila bodo izgubljene)')){ if(window.opener){window.opener.g_editedWorkOrderHTML=''; window.close();} else { g_editedWorkOrderHTML=''; document.getElementById('modal-work-order').querySelector('.editable-area').innerHTML = getWorkOrderHTML(); } }" style="background: #ef4444; color: white; border: none; padding: 8px 16px; border-radius: 4px; font-weight: bold; cursor: pointer; font-size: 13px;">PONASTAVI</button>
                    <div style="display: flex; gap: 4px; align-items: center; background: #cbd5e1; padding: 3px; border-radius: 6px;">
                        <button onclick="document.execCommand('bold', false, null)" style="background: white; border: none; border-radius: 4px; width: 28px; height: 28px; font-weight: bold; cursor: pointer; color: #000; font-size: 14px;" title="Krepko (Bold)">B</button>
                        <button onclick="document.execCommand('italic', false, null)" style="background: white; border: none; border-radius: 4px; width: 28px; height: 28px; font-style: italic; cursor: pointer; color: #000; font-size: 14px; font-family: Georgia, serif;" title="Ležeče (Italic)">I</button>
                        <button onclick="document.execCommand('underline', false, null)" style="background: white; border: none; border-radius: 4px; width: 28px; height: 28px; text-decoration: underline; cursor: pointer; color: #000; font-size: 14px;" title="Podčrtano (Underline)">U</button>
