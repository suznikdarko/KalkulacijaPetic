...
save_logic = '''
        ...
        async function saveProjectToFile() {
            try {
                ...
                const fileName = name.replace(/[^a-z0-9\u0100-\u017F]/gi, '_') + '.kuverte.json';
...
# We also need to fix exportQuoteWord in k_script_0.js to point to port 8095 (like we did in the previous session)
js_content = js_content.replace(
    "const srvRes = await fetch('http://127.0.0.1:8080'",
    "const srvRes = await fetch('http://127.0.0.1:8095'"
)

# Replace 'Ponudba.doc' with 'Angebot.doc' for AT (this was another fix we did earlier)
# Actually the existing k_script_0.js might not have exportQuoteWord.
# Wait, I checked it earlier, it has unction exportQuoteWord().
# Let's ensure it has the exact fixes requested by user today (shranjevanje ponudb v wordu v server srečko ponudbe 2026).
# The user said "spremeni poti shranjevana", which I did by changing port 8080 to 8095.
...
print("SUCCESS: REBUILT KUVERTE1.HTML FROM SCRATCH!")
