            ...
            const item = {
                ...
                quantities: qList.map(q => {
                    ...
                    return {
                        ...
                        perItem: res.pricePerItem,
                        pricePerItem: res.pricePerItem,
...
<div style="display: flex; gap: 10px; margin-top: 5px; flex-wrap: wrap;">
                                ${item.quantities.map(q => {
                                    const pTot = q.priceTotal !== undefined ? q.priceTotal : (q.total !== undefined ? q.total : (q.finalTotal !== undefined ? q.finalTotal : 0));
                                    return `<span style="background: rgba(59, 130, 246, 0.2); color: #60a5fa; padding: 2px 6px; border-radius: 4px; font-size: 0.75rem;">${formatQty(q.qty)}: ${formatPrice(pTot)}</span>`;
                                }).join('')}
                            </div>
                        </div>
                    ...
                    <div style="margin-bottom: 15px; ${idx < itemsToRender.length - 1 ? 'border-bottom: 2px dashed #e2e8f0; padding-bottom: 15px;' : ''}">
                        ...
                        <table style="width: 100%; border-collapse: collapse; margin-top: 10px; text-align: center; border-top: 1px solid #000; border-bottom: 1px solid #000;">
                            ...
                            ${(item.quantities && Array.isArray(item.quantities)) ? item.quantities.map(q => {
                                const pUnit = q.pricePerUnit !== undefined ? q.pricePerUnit : (q.perItem !== undefined ? q.perItem : (q.pricePerItem !== undefined ? q.pricePerItem : 0));
                                const pTot = q.priceTotal !== undefined ? q.priceTotal : (q.total !== undefined ? q.total : (q.finalTotal !== undefined ? q.finalTotal : 0));
                                return `
                                <tr>
                                    <td style="font-weight: bold; padding: 4px;">${formatQty(q.qty)} ${isAT ? 'Stk.' : 'kos'}</td>
                                    <td style="padding: 4px;">${formatPrice(pUnit, 3)}</td>
                                    <td style="font-weight: bold; text-align: right; padding: 4px;">${formatPrice(pTot, 2)}</td>
                                </tr>
                                `;
